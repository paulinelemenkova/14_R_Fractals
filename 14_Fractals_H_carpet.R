# Generate H-Fractals with ggplot The code below starts with setting the initial variables and defines the first line in a data frame. The data frame holds the start and end coordinates and the iteration number.
 ...
 library(ggplot2)
# Defining H-fractal parameters
# Defining number of iterations
p <- 10
# Defining shortening parameter
 a <- 1 / sqrt(2.2)
 # Defining horizontal starting line
 h_fractal <- data.frame(x1 = -1, y1 = 0, x2 = 1, y2 = 0, m = 0)
...
#    scale_colour_gradientn(colours = c("blue", "dodgerblue", "white", "orange", "red"), rescaler = ~ scales::rescale_mid(.x, mid = 0)) +
#   theme_void()
# theme_linedraw()
theme_dark()
